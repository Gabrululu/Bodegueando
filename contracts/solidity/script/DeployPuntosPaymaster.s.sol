...
/// address on every EVM chain, already confirmed deployed on Arbitrum Sepolia) and the
/// already-deployed PuntosToken, then deposits an initial ETH stake so it can start
/// sponsoring UserOperations right away.
///
/// Usage:
///   PUNTOS_TOKEN_ADDRESS=... DEPOSIT_ETH=0.02 \
...
contract DeployPuntosPaymaster is Script {
    ...
    function run() external {
        ...
        PuntosPaymaster paymaster =
            new PuntosPaymaster(IEntryPoint(ENTRY_POINT_V07), IERC20(puntosTokenAddress), deployer);
